...
class MultiRxTransform:
    ...
    def __call__(self, sample: Tuple[Tensor]) -> Tuple[Tensor]:
        x, y, t0 = sample
        return (torch.repeat_interleave(x, self.n_rx, -2), y, t0)
...
class DisAMRDataModule(pl.LightningDataModule):
    def __init__(self, dataset_path: str, batch_size, frame_size: int = 1024, n_rx: int = 1, min_snr = 0, max_snr = 13, seed: int = 42,start=0, stop=31250):
        super().__init__()
    ...
    def setup(self, stage: str = None):
        ...
        with h5py.File(self.dataset_path, "r") as f:
            x = torch.from_numpy(f['x'][self.start:self.stop])
            y = torch.from_numpy(f['y'][self.start:self.stop]).to(torch.long)
            t0 = torch.from_numpy(f['T0'][self.start:self.stop])
