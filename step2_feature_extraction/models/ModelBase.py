...
class ModelBase(pl.LightningModule):
    ...
    def on_test_epoch_end(self):
        ...
        SNRs = torch.unique(test_snr).numpy()
        F1s = []
        for snr in SNRs:
            ind = test_snr == snr
            F1s.append(torchmetrics.functional.classification.multiclass_f1_score(self.outputs_list[ind].cpu(), test_true[ind], len(self.classes)).numpy())
        self.outputs_list = self.outputs_list.zero_()
